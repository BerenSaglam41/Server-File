...
FILES_WRITER_USER="${FILES_WRITER_USER:-files-writer}"
FILES_WRITER_GROUP="${FILES_WRITER_GROUP:-files-publishers}"
...
if [ "$NFS_MODE" = "production" ]; then
  if ! getent group "$FILES_WRITER_GROUP" >/dev/null 2>&1; then
    groupadd --system "$FILES_WRITER_GROUP"
  fi
  if ! id "$FILES_WRITER_USER" >/dev/null 2>&1; then
    useradd --system --no-create-home --gid "$FILES_WRITER_GROUP" "$FILES_WRITER_USER"
  fi
  WRITER_UID="$(id -u "$FILES_WRITER_USER")"
  WRITER_GID="$(id -g "$FILES_WRITER_USER")"
  echo "Writer: $FILES_WRITER_USER:$FILES_WRITER_GROUP ($WRITER_UID:$WRITER_GID)"
fi
...
if [ "$NFS_MODE" = "production" ]; then
  echo "[..] Production sahiplik/izinleri ayarlanıyor"
  chown -R "$FILES_WRITER_USER:$FILES_WRITER_GROUP" \
    "$FILES_ROOT/export" "$FILES_ROOT/staging" "$FILES_ROOT/manifests" "$FILES_ROOT/restore-tests"
...
if [ "$NFS_MODE" = "production" ]; then
  export_line="$FILES_ROOT ${API_SERVER_IP}(rw,sync,no_subtree_check,all_squash,anonuid=${WRITER_UID},anongid=${WRITER_GID})"
else
